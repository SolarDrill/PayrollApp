...
{
    public class EmpleadosPageModel : PageModel
    {
        private readonly ApiService _apiService;
        public string baseUrl = "https://localhost:44326/api/Employees";
        public List<Employees> Employees { get; set; }
        public Employees Employee { get; set; } // Propiedad para almacenar el empleado obtenido por ID

        public EmpleadosPageModel(ApiService apiService)
        {
        ...
        public async Task OnGetAsync()
        {
            try
            {
                Employees = await _apiService.GetDataAsync<List<Employees>>(baseUrl);
            }
            catch (Exception ex)
            {
                // Manejar errores de manera adecuada, por ejemplo, registrar el error o mostrar un mensaje al usuario
                Employees = new List<Employees>(); // Inicializar la lista para evitar problemas en la vista
            }
        }
        ...
        public async Task<IActionResult> OnGetEmpleadoAsync(int id)
        {
            try
            {
                Employee = await _apiService.GetDataByIdAsync<Employees>($"{baseUrl}/{id}", id.ToString());
        ...
        public async Task<IActionResult> OnPostCrearAsync(Employees nuevoEmpleado)
        {
            try
            {
        ...
        public async Task<IActionResult> OnPostActualizarAsync(Employees empleadoActualizado)
        {
            try
            {
